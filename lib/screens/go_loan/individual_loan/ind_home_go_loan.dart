import 'package:capex/components/drawer.dart';
import 'package:flutter/material.dart';
class IndHomeGoLoan extends StatefulWidget {
  @override
  _IndHomeGoLoanState createState() => _IndHomeGoLoanState();
}


class _IndHomeGoLoanState extends State<IndHomeGoLoan> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          'Home',
        ),
      ),
      drawer: MyDrawer(),
      body: Center(
        child: SingleChildScrollView(
          child: Column(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            crossAxisAlignment: CrossAxisAlignment.center,
            mainAxisSize: MainAxisSize.max,
            children: [
              InkWell(
                onTap: (){
                  Navigator.pushReplacementNamed(
                    context,
                    '/IndNewLoanCurvedNavigationBar',
                  );
                },
                child : _buildCard('assets/images/NEWLOAN.png','New Loan'),
              ),
              InkWell(
                onTap: (){
                  Navigator.pushReplacementNamed(
                    context,
                    '/IndStatusCurvedNavigationBar',
                  );
                },
                child : _buildCard('assets/images/STATUS.png','Status'),
              ),
              InkWell(
                onTap: (){
                  Navigator.pushReplacementNamed(
                    context,
                    '/IndMyProfileCurvedNavigationBar',
                  );
                },
                child : _buildCard('assets/images/PROFILE.png','Profile'),
              )
            ],
          ),
        ),

      ),
    );
  }
}

Widget _buildCard(String image , String text) {
  return Container(
    height: 150,
    width: double.infinity,
    margin: EdgeInsets.fromLTRB(5, 10, 5, 3),
    padding: const EdgeInsets.fromLTRB(10, 10, 10, 0),
    child: Card(
      elevation: 3,
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(30),
      ),
      child: Stack(
        children: [
          Positioned(
              top: 5,
              right: 20,
              child: Image.asset(
                image,
                fit: BoxFit.cover,
                width: 150,
                height: 120,
              )
          ),
          Positioned(
            top: 55,
            left: 25,
            child: Text(
              text,
              style: TextStyle(
                color: Color(0xff3d5a96),
                fontSize: 25,
              ),
            ),
          ),
        ],
      ),
    ),
  );
}
